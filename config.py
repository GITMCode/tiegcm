...
def _write_make_env(builddir, make_fragment, srcdir, exe_path,
                    coupling=False, hidra=False, debug=False):
    content = (
...
def _write_root_makefile(tiegcmhome, tiegcmdata, gswm_res):
    content = "\n".join([
        ...
        "",
        f"TIEGCMHOME = {tiegcmhome}",
        f"TIEGCMDATA = {tiegcmdata}",
        f"GSWM_RES   = {gswm_res}",
        "NAME      ?= run",
        "",
...
def configure(args):
    ...
    _write_make_env(
        builddir, make_fragment, srcdir, exe_path, coupling, hidra, debug
    )
    ...
    print(f"  Debug={debug}  Coupling={coupling}  HIDRA={hidra}")
    if defs_changed:
...
def show_status():
    ...
    print(f"  Debug={env.get('DEBUG','?')}  Coupling={env.get('COUPLING','?')}  HIDRA={env.get('HIDRA','?')}")
    print()
...
def uninstall():
    ...
    targets = (
        [os.path.join(_TIEGCMHOME, "Makefile")]
